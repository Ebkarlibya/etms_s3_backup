...
def take_backup():
	...
	enqueue(
		"etms_s3_backup.etms_s3_backup.doctype.etms_s3_backup.etms_s3_backup.take_backups_s3",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if cint(frappe.db.get_single_value("ETMS S3 Backup", "enabled")):
		if frappe.db.get_single_value("ETMS S3 Backup", "frequency") == freq:
			take_backups_s3()	
...
def take_backups_s3(retry_count=0):
	try:
		validate_file_size()
		backup_to_s3()
		send_email(True, "Amazon S3", "ETMS S3 Backup", "notify_email")
...
def notify():
	...
	send_email(False, "Amazon S3", "ETMS S3 Backup", "notify_email", error_message)
...
def backup_to_s3():
	...
	doc = frappe.get_single("ETMS S3 Backup")
	bucket = doc.bucket
